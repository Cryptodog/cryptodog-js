...
function Multiparty(opts) {
  if (opts.privateKey) {
    ...
    this.privateKey = enc.decodeFromBase64(opts.privateKey);
  } else {
    this.privateKey = crypt.nacl.randomBytes(32);
  }

  this._cb = opts.onOutput;

  if (!this._cb) {
    throw new Error("Multiparty constructor object must have 'onOutput' transmission callback")
  }
...
Multiparty.prototype.sendMessage = function(msg) {
  var tagg = crypt.nacl.randomBytes(64);
  var msg = new etc.Buffer(enc.decodeFromUTF8(msg));
  msg.writeBytes(tagg);
...
Multiparty.prototype.receiveMessage = function(nick, msg) {
  ...
  switch (obj.type) {
    ...
    if (_strcmp(pHMAC, rHMAC) !== 0) {
      console.log(enc.encodeToHex(pHMAC), enc.encodeToHex(rHMAC));
    ...
    var byts = plaintext.slice(0, plaintext.length-64);
    this.emit("message", { from: nick, body: enc.encodeToUTF8(byts) });
    return null;
    break;
  }
} 
