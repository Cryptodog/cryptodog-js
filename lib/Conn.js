...
Conn.prototype._bexGroup = function(evt) {
  ...
  packets.map(function(packet) {
    switch (packet.header) {
      ...
      case Op.FLAG_ME_AS_BOT:
      $this.groupConversations[evt.room].peers[evt.from]._bot = true;
      break;
...
Conn.prototype._bexPrivate = function(evt) {
  ...
  packets.map(function(packet) {
    switch (packet.header) {
      ...
      case Op.PING:
      $this.sendBexPrivate(evt.room, evt.room, [{
        header: Op.PONG,
...
Conn.prototype.isABot = function(room, nick) {
  if (!this.groupConversations[room]) {
    return false;
  }

  if (!this.groupConversations[room].peers[nick]) {
    return false;
  }

  return this.groupConversations[room].peers[nick]._bot === true;
}
  ...
  $this.store.getItem("rooms").then(function(rooms) {
    ...
    $this._joinCb[r] = function() {
      ...
      gc.on("newkey", function(evt) {
        ...
        // Check if this peer's fingerprints have changed
        if ($this.authState[r]) {
...
Conn._b64 = /^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/;
...
Conn.prototype.setupOTR = function(room, from) {
  ...
  o.on("ui", function(msg, encrypted, meta) {
    if (Conn._b64.test(msg)) {
      var purportedBex = etc.Encoding.decodeFromBase64(msg);
      if (BEX.headerBytes(purportedBex)) { 
        $this.emit("dm-binary", {
          room: room,
          from: from,
          body: purportedBex
...
Conn.prototype._reconnect = function() {
  var $this = this;
  ...
  $this._stanza.on("connected", function() {
    if ($this.expectingReconnect) {
      $this.emit("reconnected");
    }
  })

  $this._stanza.on("disconnected", function() {
    console.log("disconnected");
    if ($this.expectedDC === false) {
